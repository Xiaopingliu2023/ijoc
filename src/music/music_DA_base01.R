...
data <- read.table("cex_basefile97_02_my.csv",header=TRUE, quote="", sep=",", na.strings="?")

if(0==1){
data <- read.table("black-friday-train.csv",header=TRUE, quote="", sep=",", na.strings="?")
d<-data
d[is.na(d)] <- 0
summary(d)
}
...
#my_data<-data[data$cdall>0,c(7, 8, 11,12, 6)]
